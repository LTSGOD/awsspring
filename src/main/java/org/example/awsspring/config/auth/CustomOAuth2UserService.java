...
import org.example.awsspring.config.auth.dto.OAuthAttributes;
import org.example.awsspring.config.auth.dto.SessionUser;
import org.example.awsspring.domain.user.User;
import org.example.awsspring.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		String registrationId = userRequest.getClientRegistration().getRegistrationId();//네이버, 구글 구분

		String userNameAttributeName = userRequest.getClientRegistration()
			.getProviderDetails()
			.getUserInfoEndpoint()
			.getUserNameAttributeName();
		//로그인 진행시 키가 되는 필드값 pk와 같음.
